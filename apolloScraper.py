...
def get_the_emails(driver,elem):
    ...
    try:
        ...
        if_elem_not_hidden = WebDriverWait(driver,10).until(EC.element_to_be_clickable((By.XPATH,"//div[@class='zp-contact-email-envelope-container zp_C2NVB zp_pis85']//div/a")))
...
def get_different_tbodies(driver,get_rows):
    ...
    try:
        table_rows_datas = get_rows.find_elements(By.CLASS_NAME,'zp_aBhrx') #zp_1sEIg
    except:
        print('no table_rows_datas')
    try:
        responsible_person = table_rows_datas[0].find_element(By.CLASS_NAME,value="zp_xVJ20") #zp_EqOJn
...
def get_the_precise_details_of_company(driver):
    ...
    try:
        ...
        #     EC.presence_of_all_elements_located((By.CLASS_NAME,"zp_RFed0")))  #zp_lXHd4

        get_tables =  WebDriverWait(driver, 10).until(
            EC.presence_of_all_elements_located((By.XPATH,"//div[@class='zp_DUflC']//table//tbody")))  #zp_lXHd4
...
def get_details_of_company(driver):
    static_requirement_of_client = ['CTO','Talent acquisition','CFO','Recruiter','CEO', 'Tech Lead']
...
def check_if_revenue_matches_criteria(driver):
    try:
        ...
        print(checkRevenue)
        if checkRevenue[-1] == 'B' or (checkRevenue[-1] == 'M' and int(checkRevenue[1:len(checkRevenue)-1]) > 100):
            return False
        else:
            return True
...
def find_if_eligible_company(driver,list_of_companiesDetails):
    ...
    wb = load_workbook(filename=cloudengineer)
    ws = wb.active
    ...
    wb.save(filename = cloudengineer)
