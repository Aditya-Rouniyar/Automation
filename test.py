...
def readFile(fileName):
    ...
    getExperience = []
    get_tech_stack = []
    tech_stacks = ['AWS','Azure', 'GCP','Docker', 'Ansible', 'Python', 'Ruby',  '.NET',  'C#', 'Go', 'SQL', 'MySQL', 'PostgreSQL', 'MongoDB', 'Terraform', 'Jenkins', 'Puppet', 'Chef']
    ...
    for word in words:
        if word.find('U.S') != -1 or word.find('U.S work permit') != -1 or word.find('USA') != -1 or word.find('U.S permit') != -1 or word.find('U.S authorization') != -1:
            getInfo.append(True)
            return getInfo
